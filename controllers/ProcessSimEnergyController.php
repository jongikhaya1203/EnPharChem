<?php
/**
 * EnPharChem - Process Simulation for Energy Controller
 * Handles energy process simulation modules (benchmarked against AspenTech HYSYS)
 */

class ProcessSimEnergyController extends BaseController {

    private $categorySlug = 'process-sim-energy';
    ...
    public function index() {
        ...
        $this->view('modules/category', [
            'pageTitle' => 'Process Simulation for Energy',
            'category' => $category,
            'modules' => $modules,
        ]);
    }

    public function hysys() {
        $module = $this->db->fetch(
            "SELECT m.*, mc.name as category_name, mc.slug as category_slug
             ...
             WHERE mc.slug = ? AND m.slug LIKE '%hysys%'",
            [$this->categorySlug]
        );
    ...
    public function module() {
        $slug = $_GET['slug'] ?? '';
        if (empty($slug)) {
            $this->redirect('process-sim-energy');
        }
